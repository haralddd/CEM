...
- `dq::Float64`: Wave number spacing [1/m]
- `Lx::Float64`: Surface length [m]
- `dx::Float64`: Surface point spacing [m]
...
function RayleighParams(;
    ...
    return RayleighParams{SurfType}(plan_fft!(similar(xs, ComplexF64)),
        xs, ps, qs, ks, kis,
        nu, eps, mu, lambda, omega,
        Q, dq, Lx, dx,
        Ni, Nq, Nx, scale(surf, omega/c0), seed, Xoshiro(seed))
end
...
function scaled_params(rp::RayleighParams)::Dict
    return Dict(
        ...
        :omega => rp.omega,
        :Q => rp.Q,
        :ks => rp.ks * rp.omega / c0,
        :Nq => rp.Nq,
        :Lx => rp.Lx * c0 / rp.omega,
        :Ni => rp.Ni,
        :surf => scale(rp.surf, c0 / rp.omega),
    )
end
